...
class BaseAttributeServer {
  constructor(id, name, readAcl, writeAcl, schema, isWritable, isSubscribable, requiresTimedInteraction, initValue, defaultValue) {
    ...
    try {
      ...
      logger.warn(
        `Attribute value to initialize for ${name} has an invalid value ${import_general.Logger.toJSON(
          initValue
        )}. Restore to default ${import_general.Logger.toJSON(defaultValue)}`
      );
...
class AttributeServer extends FixedAttributeServer {
  ...
  processSet(value, session, message, delayChangeEvents = false) {
    ...
    if (delayChangeEvents) {
      ...
      logger.info(`Delay change for attribute "${this.name}" with value ${import_general.Logger.toJSON(value)}`);
    } else {
  ...
  triggerDelayedChangeEvents() {
    if (this.delayedChangeData !== void 0) {
      ...
      logger.info(`Trigger delayed change for attribute "${this.name}" with value ${import_general.Logger.toJSON(newValue)}`);
...
class FabricScopedAttributeServer extends AttributeServer {
  ...
  setRemote(value, session, message, delayChangeEvents = false, preserveFabricIndex = false) {
    value = this.schema.injectField(
      ...
      () => !preserveFabricIndex
      // Noone should send any index and if we simply SHALL ignore it,  biuut internally we might need it
    );
    logger.info(`Set remote value for fabric scoped attribute "${this.name}" to ${import_general.Logger.toJSON(value)}`);
