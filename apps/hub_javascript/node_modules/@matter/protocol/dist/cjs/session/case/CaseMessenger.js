...
class CaseServerMessenger extends import_SecureChannelMessenger.SecureChannelMessenger {
  async readSigma1() {
    const { payload } = await this.nextMessage("CASE Sigma1", import_types.SecureMessageType.Sigma1);
  ...
  async readSigma3() {
    const { payload } = await this.nextMessage("CASE Sigma3", import_types.SecureMessageType.Sigma3);
