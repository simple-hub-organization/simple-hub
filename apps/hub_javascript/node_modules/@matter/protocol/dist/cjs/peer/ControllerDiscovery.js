...
class ControllerDiscovery {
  ...
  static async discoverDeviceAddressesByIdentifier(scanners, identifier, timeoutSeconds = 30) {
    logger.info(`Start Discovering devices using identifier ${import_general.Logger.toJSON(identifier)} ...`);
    const scanResults = scanners.map(async (scanner) => {
      ...
      logger.info(`Found ${foundDevices.length} devices using identifier ${import_general.Logger.toJSON(identifier)}`);
      if (foundDevices.length === 0) {
        throw new import_ControllerCommissioningFlow.CommissioningError(
          `No device discovered using identifier ${import_general.Logger.toJSON(
            identifier
      ...
      if (devices.length === 0) {
        throw new import_ControllerCommissioningFlow.CommissioningError(
          `Device discovered using identifier ${import_general.Logger.toJSON(
            identifier
