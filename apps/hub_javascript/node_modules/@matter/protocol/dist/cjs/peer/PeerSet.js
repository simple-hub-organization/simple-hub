...
class PeerSet {
  ...
  #construction;
  #store;
  #interactionQueue = new import_general.PromiseQueue(CONCURRENT_QUEUED_INTERACTIONS, INTERACTION_QUEUE_DELAY_MS);
   ...
   */
  async connect(address, discoveryOptions, allowUnknownPeer = false) {
  ...
  async #ensureConnection(address, discoveryOptions, allowUnknownPeer = false) {
    ...
    if (!this.#channels.hasChannel(address)) {
      ...
      this.#resume(address, discoveryOptions).then((channel) => {
  ...
  async initializeInteractionClient(address, discoveryOptions) {
    ...
    const client = new import_InteractionClient.InteractionClient(
      new import_ExchangeProvider.ReconnectableExchangeProvider(this.#exchanges, this.#channels, address, async () => {
        ...
        if (!this.#channels.hasChannel(address)) {
          throw new import_MessageExchange.RetransmissionLimitReachedError(
            `Device ${(0, import_PeerAddress.PeerAddress)(address)} is currently not reachable.`
          );
        }
        ...
        if (operationalAddress === void 0) {
          ...
          throw new import_MessageExchange.RetransmissionLimitReachedError(
            `No operational address found for ${(0, import_PeerAddress.PeerAddress)(address)}`
          );
        }
   ...
   */
  async #resume(address, discoveryOptions) {
    const { discoveryType } = discoveryOptions ?? {};
    const operationalAddress = discoveryType === 0 /* None */ ? this.#getLastOperationalAddress(address) : this.#knownOperationalAddressFor(address);
    try {
      return await this.#connectOrDiscoverNode(address, operationalAddress, discoveryOptions);
    } catch (error) {
      if ((error instanceof import_ControllerDiscovery.DiscoveryError || error instanceof import_general.NoResponseTimeoutError) && this.#peersByAddress.has(address)) {
  ...
  async #connectOrDiscoverNode(address, operationalAddress, discoveryOptions = {}) {
    ...
    return await (0, import_general.anyPromise)(discoveryPromises).finally(() => this.#runningPeerDiscoveries.delete(address));
  }
  ...
  #handleResubmissionStarted(session) {
    ...
    this.#scanners.scannerFor(import_general.ChannelType.UDP)?.findOperationalDevice(fabric, nodeId, RETRANSMISSION_DISCOVERY_TIMEOUT_MS, true).catch((error) => {
