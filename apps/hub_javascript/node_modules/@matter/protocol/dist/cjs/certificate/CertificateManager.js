...
((CertificateManager2) => {
  ...
  function verifyRootCertificate(rootCert) {
    ...
    if (rootCert.subject.fabricId !== void 0) {
      if (Array.isArray(rootCert.subject.fabricId)) {
        throw new CertificateError(
          `Invalid fabricId in NoC certificate: ${import_general.Logger.toJSON(rootCert.subject.fabricId)}`
        );
      }
    ...
    if (rootCert.subject.rcacId === void 0 || Array.isArray(rootCert.subject.rcacId)) {
      throw new CertificateError(`Invalid rcacId in Root certificate: ${import_general.Logger.toJSON(rootCert.subject.rcacId)}`);
    }
  ...
  function verifyNodeOperationalCertificate(rootOrIcaCert, nocCert) {
    ...
    if (nocCert.subject.nodeId === void 0 || Array.isArray(nocCert.subject.nodeId)) {
      throw new CertificateError(`Invalid nodeId in NoC certificate: ${import_general.Logger.toJSON(nocCert.subject.nodeId)}`);
    }
    ...
    if (nocCert.subject.fabricId === void 0 || Array.isArray(nocCert.subject.fabricId)) {
      throw new CertificateError(
        `Invalid fabricId in NoC certificate: ${import_general.Logger.toJSON(nocCert.subject.fabricId)}`
      );
    }
    ...
    if (rootOrIcaCert.subject.fabricId !== void 0 && rootOrIcaCert.subject.fabricId !== nocCert.subject.fabricId) {
      throw new CertificateError(
        `FabricId in NoC certificate does not match the fabricId in the parent certificate. ${import_general.Logger.toJSON(
          rootOrIcaCert.subject.fabricId
        )} !== ${import_general.Logger.toJSON(nocCert.subject.fabricId)}`
      );
    }
    ...
    if (nocCert.extensions.extendedKeyUsage === void 0 || !nocCert.extensions.extendedKeyUsage.includes(1) && !nocCert.extensions.extendedKeyUsage.includes(2)) {
      throw new CertificateError(
        `Noc certificate must have extendedKeyUsage with serverAuth and clientAuth: ${import_general.Logger.toJSON(nocCert.extensions.extendedKeyUsage)}`
      );
    }
    ...
    if (!import_general.Bytes.areEqual(nocCert.extensions.authorityKeyIdentifier, rootOrIcaCert.extensions.subjectKeyIdentifier)) {
      throw new CertificateError(
        `Noc certificate authorityKeyIdentifier must be equal to Root/Ica subjectKeyIdentifier.`
      );
    }
    import_general.Crypto.verify(
      (0, import_general.PublicKey)(rootOrIcaCert.ellipticCurvePublicKey),
  ...
  function verifyIntermediateCaCertificate(rootCert, icaCert) {
    ...
    if (icaCert.subject.fabricId !== void 0) {
      if (Array.isArray(icaCert.subject.fabricId)) {
        throw new CertificateError(
          `Invalid fabricId in NoC certificate: ${import_general.Logger.toJSON(icaCert.subject.fabricId)}`
        );
      }
      ...
      if (rootCert.subject.fabricId !== icaCert.subject.fabricId) {
        throw new CertificateError(
          `FabricId in Ica certificate does not match the fabricId in the parent certificate. ${import_general.Logger.toJSON(
            rootCert.subject.fabricId
          )} !== ${import_general.Logger.toJSON(icaCert.subject.fabricId)}`
        );
      }
    }
    if (icaCert.subject.icacId === void 0 || Array.isArray(icaCert.subject.icacId)) {
      throw new CertificateError(`Invalid icacId in Ica certificate: ${import_general.Logger.toJSON(icaCert.subject.icacId)}`);
    }
    ...
    if (rootCert.subject.rcacId !== icaCert.issuer.rcacId) {
      throw new CertificateError(
        `RcacId in Ica certificate does not match the rcacId in the parent certificate. ${import_general.Logger.toJSON(
          rootCert.subject.rcacId
        )} !== ${import_general.Logger.toJSON(icaCert.issuer.rcacId)}`
      );
    }
