...
class ExchangeManager {
  ...
  static [import_general.Environmental.create](env) {
    const instance = new ExchangeManager({
      transportInterfaces: env.get(import_general.TransportInterfaceSet),
  ...
  addProtocolHandler(protocol) {
    if (this.hasProtocolHandler(protocol.getId())) {
      throw new import_general.ImplementationError(`Handler for protocol ${protocol.getId()} already registered.`);
    }
  ...
  async onMessage(channel, messageBytes) {
    ...
    if (packet.header.sessionType === import_MessageCodec.SessionType.Group)
      throw new import_general.NotImplementedError("Group messages are not supported");
    let session;
    ...
    if (exchange !== void 0) {
      ...
      if (protocolHandler !== void 0 && message.payloadHeader.isInitiatorMessage && !isDuplicate) {
        if (message.payloadHeader.messageType == import_types.SecureMessageType.StandaloneAck && !message.payloadHeader.requiresAck) {
          logger.debug(
        ...
        if (isDuplicate) {
          ...
          logger.info(
            `Discarding unexpected message ${messageId} for protocol ${message.payloadHeader.protocolId}, exchangeIndex ${exchangeIndex} and sessionId ${session.id} : ${import_general.Logger.toJSON(message)}`
          );
        }
      }
    }
  ...
  #cleanupSessionExchanges(sessionId) {
    if (sessionId === import_SessionManager.UNICAST_UNSECURE_SESSION_ID) {
      return;
    }
  ...
  #addListener(transportInterface) {
    const udpInterface = transportInterface instanceof import_general.UdpInterface;
    this.#listeners.set(
      transportInterface,
...
class ExchangeCounter {
  exchangeCounter = import_general.Crypto.getRandomUInt16();
