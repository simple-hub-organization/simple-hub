...
class DeviceAdvertiser {
    ...
    if (fabrics.length) {
      ...
      for (const fabric of fabrics) {
        ...
        if (session === void 0 || !session.isSecure || session.subscriptions.size === 0) {
          ...
          logger.debug("Announcing", import_general.Diagnostic.dict({ fabric: fabric.fabricId }));
        }
      }
      for (const broadcaster of this.#broadcasters) {
  ...
  async clearBroadcasters() {
    ...
    if (errors.length) {
      throw new import_general.MatterAggregateError(errors, "Error closing broadcasters");
    }
  }
}
