...
class InteractionServer {
  ...
  getId() {
    return import_types.INTERACTION_PROTOCOL_ID;
  }
  get isClosing() {
          ...
          if (versionFilterValue !== void 0 && versionFilterValue === version) {
            logger.debug(
              `Read attribute from ${exchange.channel.name}: ${this.#endpointStructure.resolveAttributeName(
                path
              )}=${import_general.Logger.toJSON(value)} (version=${version}) ignored because of dataVersionFilter`
            );
            continue;
          }
          logger.debug(
            `Read attribute from ${exchange.channel.name}: ${this.#endpointStructure.resolveAttributeName(
              path
            )}=${import_general.Logger.toJSON(value)} (version=${version})`
          );
        ...
        for (const { path, event } of events) {
          try {
            ...
            logger.debug(
              `Read event from ${exchange.channel.name}: ${this.#endpointStructure.resolveEventName(
                path
              )}=${import_general.Logger.toJSON(matchingEvents)}`
            );
            const { schema } = event;
            reportsForPath.push(
  ...
  async handleWriteRequest(exchange, { suppressResponse, timedRequest, writeRequests, interactionModelRevision, moreChunkedMessages }, message) {
    ...
    for (const writeRequest of writeData) {
      ...
      for (const { path, attribute } of attributes) {
        ...
        try {
          ...
          logger.debug(
            `Handle write request from ${exchange.channel.name} resolved to: ${this.#endpointStructure.resolveAttributeName(path)}=${import_general.Logger.toJSON(
              value
    ...
    logger.debug(
      `Write request from ${exchange.channel.name} done ${errorResults.length ? `with following errors: ${errorResults.map(
        ({ path, statusCode }) => `${this.#endpointStructure.resolveAttributeName(path)}=${import_general.Logger.toJSON(statusCode)}`
      ).join(", ")}` : "without errors"}`
    );
    const response = {
      interactionModelRevision: import_model.Specification.INTERACTION_MODEL_REVISION,
