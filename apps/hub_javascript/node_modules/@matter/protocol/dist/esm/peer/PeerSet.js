...
class PeerSet {
  ...
  #construction;
  #store;
  #interactionQueue = new PromiseQueue(CONCURRENT_QUEUED_INTERACTIONS, INTERACTION_QUEUE_DELAY_MS);
  ...
  async #resume(address, discoveryOptions) {
    ...
    try {
      ...
      if ((error instanceof DiscoveryError || error instanceof NoResponseTimeoutError) && this.#peersByAddress.has(address)) {
  ...
  #handleResubmissionStarted(session) {
    ...
    this.#scanners.scannerFor(ChannelType.UDP)?.findOperationalDevice(fabric, nodeId, RETRANSMISSION_DISCOVERY_TIMEOUT_MS, true).catch((error) => {
