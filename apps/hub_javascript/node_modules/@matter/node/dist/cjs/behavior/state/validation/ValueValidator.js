  ...
  switch (metatype) {
    ...
    case void 0:
      ...
      if (type === void 0) {
        ...
        throw new import_errors.SchemaImplementationError((0, import_model.DataModelPath)(schema.path), `No type defined`);
      }
...
function createEnumValidator(schema, supervisor) {
  ...
  return (value, session, location) => {
    ...
    if (!valid.has(value)) {
      throw new import_errors.DatatypeError(location, "defined in enum", value, import_types.StatusCode.ConstraintError);
    }
...
function createBitmapValidator(schema, supervisor) {
  ...
  for (const field of supervisor.membersOf(schema)) {
    ...
    if (field?.parent?.id === import_model.FeatureMap.id) {
      name = (0, import_general.camelize)(field.description ?? field.name);
    } else {
