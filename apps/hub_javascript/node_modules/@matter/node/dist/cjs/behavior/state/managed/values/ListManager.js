...
function createConfig(owner, schema) {
  ...
  if (entry === void 0) {
    throw new import_errors.SchemaImplementationError((0, import_model.DataModelPath)(schema.path), "List schema has no entry definition");
  }
...
function createProxy(config, reference, session) {
  ...
  const writeVal = () => {
    if (reference.expired) {
      throw new import_errors.ExpiredReferenceError(reference.location);
    }
  ...
  let setListLength = (length) => {
    if (length > 65535) {
      throw new import_errors.WriteError(reference.location, `Index ${length} is greater than allowed maximum of 65535`);
    }
  ...
  if (config.manageEntries) {
    readEntry = (index) => {
      ...
      if (index < 0 || index >= readVal().length) {
        throw new import_errors.ReadError(reference.location, `Index ${index} is out of bounds`);
      }
      if (index > 65535) {
        throw new import_errors.ReadError(reference.location, `Index ${index} is greater than allowed maximum of 65535`);
      }
    ...
    readEntry = (index, location) => {
      ...
      if (index < 0 || index > readVal().length) {
        throw new import_errors.WriteError(location, `Index ${index} is out of bounds`);
      }
  ...
  let writeEntry = (index, value, location) => {
    ...
    if (index > 65535) {
      throw new import_errors.ReadError(location, `Index ${index} is greater than allowed maximum of 65535`);
    }
  ...
  if (config.fabricScoped) {
    let mapScopedToActual2 = function(index, reading) {
      if (index < 0) {
        throw new (reading ? import_errors.ReadError : import_errors.WriteError)(reference.location, `Negative index ${index} unsupported`);
      }
      let nextPos = 0;
      ...
      if (reading) {
        throw new import_errors.ReadError(reference.location, `Index ${index} extends beyond available entries`);
      }
      if (nextPos === index) {
        return readVal().length;
      }
      throw new import_errors.WriteError(reference.location, `Index ${index} would leave gaps in fabric-filtered list`);
    };
      ...
      writeEntry = (index, value, location) => {
        if (value === void 0) {
          ...
          if (typeof value !== "object") {
            throw new import_errors.WriteError(location, `Fabric scoped list value is not an object`, import_types.StatusCode.Failure);
          }
