 ...
 */
const logger = import_general.Logger.get("Transaction");
...
function act(via, actor) {
  ...
  const handleTransactionError = (error) => {
    ...
    logger.error("Rolling back", tx.via, "due to error:", import_general.Diagnostic.weak(error?.message || `${error}`));
    try {
  ...
  try {
    ...
    if (import_general.MaybePromise.is(actorResult)) {
      isAsync = true;
    ...
    if (import_general.MaybePromise.is(commitResult)) {
      isAsync = true;
...
class Tx {
  ...
  constructor(via, readonly = false) {
    this.#via = import_general.Diagnostic.via(via);
    if (readonly) {
  ...
  onShared(listener, once) {
    if (this.#shared === void 0) {
      this.#shared = (0, import_general.Observable)();
    }
  ...
  onClose(listener) {
    ...
    if (this.#closed === void 0) {
      this.#closed = (0, import_general.Observable)();
    }
  ...
  addParticipants(...participants) {
    ...
    for (const participant of participants) {
      ...
      if ([...this.#participants].findIndex((p) => p.toString() === participant.toString()) !== -1) {
        throw new import_general.ImplementationError(`Participant ${participant} identity is not unique`);
      }
  ...
  waitFor(others) {
    ...
    logger.debug(
      ...
      this.via,
      "waiting on",
      (0, import_general.describeList)("and", ...[...others].map((other) => other.via))
    );
  ...
  #executePreCommit() {
    ...
    const errorRollback = (error) => {
      ...
      if (import_general.MaybePromise.is(result)) {
        return result.then(() => {
          import_types.StatusResponseError.reject(error);
          throw new import_Errors.FinalizationError("Rolled back due to pre-commit error");
        });
      }
      import_types.StatusResponseError.reject(error);
      throw new import_Errors.FinalizationError("Rolled back due to pre-commit error");
  ...
  #executeCommit2() {
    ...
    let ongoing;
    for (const participant of this.participants) {
      const promise = import_general.MaybePromise.then(
        () => participant.commit2(),
        void 0,
        (error) => {
  ...
  #locksChanged(resources, how = "locked") {
    ...
    if (how === "locked") {
      resourceDescription = import_general.Diagnostic.strong((0, import_general.describeList)("and", ...[...resources].map((r) => r.toString())));
    } else {
  ...
  #assertAvailable() {
    ...
    if (this.#status === import_Status.Status.ReadOnly) {
      throw new import_general.ReadOnlyError();
    }
  }
}
...
function throwIfErrored(errored, when) {
  ...
  throw new import_Errors.FinalizationError(
    `Unhandled error${suffix} ${when} participant${suffix} ${(0, import_general.describeList)(
      "and",
