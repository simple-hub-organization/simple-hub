...
function StructPatcher(schema, supervisor) {
  ...
  return (changes, target, path) => {
    if (typeof changes !== "object" || changes === null || Array.isArray(changes)) {
      throw new import_errors.WriteError(path, `patch definition ${changes} is not an object`);
    }
    if (typeof target !== "object" || target === null || Array.isArray(target)) {
      throw new import_errors.WriteError(path, `cannot patch ${target} because it is not an object`);
    }
    for (const key in changes) {
      if (!(key in memberPatchers)) {
        throw new import_errors.WriteError(path, `${key} is not a property of ${schema.name}`);
      }
  ...
  return (changes, target, path) => {
    ...
    for (const indexStr in changes) {
      ...
      if (index < 0 || Number.isNaN(index)) {
        throw new import_errors.WriteError(path, `key ${index} is not a valid array index`);
      }
