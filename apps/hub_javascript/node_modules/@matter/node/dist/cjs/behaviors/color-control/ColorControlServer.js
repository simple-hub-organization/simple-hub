...
const ColorControlServerBase = import_ColorControlBehavior.ColorControlBehavior.with(
...
class ColorControlServerLogic extends ColorControlServerBase {
  ...
  initialize() {
    ...
    if (this.features.colorTemperature && this.#getBootReason() !== import_general_diagnostics2.GeneralDiagnostics.BootReason.SoftwareUpdateCompleted) {
  ...
  moveHueLogic(moveMode, rate, isEnhancedHue = false) {
    ...
    if (!this.state.managedTransitionTimeHandling) {
      this.internal.stopHueTransition();
      if (isEnhancedHue) {
        this.state.enhancedCurrentHue = (0, import_general.addValueWithOverflow)(
        ...
        this.state.currentHue = (0, import_general.addValueWithOverflow)(this.state.currentHue, rate, MIN_HUE_VALUE, maxHue);
      }
      this.state.remainingTime = 0;
      return;
    }
    this.#initiateTransition(
      isEnhancedHue ? "enhancedHue" : "hue",
      rate * (moveMode === import_color_control.ColorControl.MoveMode.Up ? 1 : -1),
      MIN_HUE_VALUE,
      maxHue
    );
  }
  ...
  stepHueLogic(stepMode, stepSize, transitionTime, isEnhancedHue = false) {
    const currentHue = isEnhancedHue ? this.state.enhancedCurrentHue : this.state.currentHue;
    const maxHue = isEnhancedHue ? MAX_ENHANCED_HUE_VALUE : MAX_HUE_VALUE;
    const targetValue = (0, import_general.addValueWithOverflow)(
      currentHue,
      stepMode === import_color_control.ColorControl.StepMode.Up ? stepSize : -stepSize,
      MIN_HUE_VALUE,
      maxHue
    );
    if (!this.state.managedTransitionTimeHandling || transitionTime === 0) {
    ...
    const effectiveRate = Math.ceil(stepSize / transitionTime * 10) * (stepMode === import_color_control.ColorControl.StepMode.Up ? 1 : -1);
    this.#initiateTransition(
      isEnhancedHue ? "enhancedHue" : "hue",
      effectiveRate,
      MIN_HUE_VALUE,
  ...
  moveSaturationLogic(moveMode, rate) {
    if (!this.state.managedTransitionTimeHandling) {
      this.internal.stopSaturationTransition();
      this.state.currentSaturation = moveMode === import_color_control.ColorControl.MoveMode.Up ? MAX_SATURATION_VALUE : MIN_SATURATION_VALUE;
      this.state.remainingTime = 0;
      return;
    }
    this.#initiateTransition(
  ...
  stepSaturationLogic(stepMode, stepSize, transitionTime) {
    ...
    const targetValue = (0, import_general.cropValueRange)(
      stepMode === import_color_control.ColorControl.StepMode.Up ? this.state.currentSaturation + stepSize : this.state.currentSaturation - stepSize,
      MIN_SATURATION_VALUE,
      MAX_SATURATION_VALUE
    );
  ...
  moveToHueAndSaturationLogic(targetHue, targetSaturation, transitionTime) {
    return import_general.MaybePromise.then(
      this.moveToHueLogic(targetHue, import_color_control.ColorControl.Direction.ShortestDistance, transitionTime, false),
  ...
  moveToColorLogic(targetX, targetY, transitionTime) {
    if (!this.state.managedTransitionTimeHandling || transitionTime === 0) {
      ...
      this.state.currentX = (0, import_general.cropValueRange)(targetX, MIN_CIE_XY_VALUE, MAX_CIE_XY_VALUE);
      this.state.currentY = (0, import_general.cropValueRange)(targetY, MIN_CIE_XY_VALUE, MAX_CIE_XY_VALUE);
      this.state.remainingTime = 0;
      return;
    }
  ...
  stepColorLogic(stepX, stepY, transitionTime) {
    const targetValueX = (0, import_general.cropValueRange)(this.state.currentX + stepX, MIN_CIE_XY_VALUE, MAX_CIE_XY_VALUE);
    const targetValueY = (0, import_general.cropValueRange)(this.state.currentY + stepY, MIN_CIE_XY_VALUE, MAX_CIE_XY_VALUE);
    if (!this.state.managedTransitionTimeHandling || transitionTime === 0) {
      this.internal.stopXTransition();
      this.internal.stopYTransition();
  ...
  moveToEnhancedHueAndSaturationLogic(targetEnhancedHue, targetSaturation, transitionTime) {
    return import_general.MaybePromise.then(
      this.moveToHueLogic(targetEnhancedHue, import_color_control.ColorControl.Direction.ShortestDistance, transitionTime, true),
      () => this.moveToSaturationLogic(targetSaturation, transitionTime)
    );
  }
  /**
  ...
  colorLoopSet({
    ...
    if (updateFlags.updateAction) {
      if (action === import_color_control.ColorControl.Action.DeActivateTheColorLoop) {
        ...
        return import_general.MaybePromise.then(
          ...
          () => {
            ...
            if (action === import_color_control.ColorControl.Action.ActivateTheColorLoopFromTheValueInTheColorLoopStartEnhancedHueField) {
              return this.startColorLoopLogic(this.state.colorLoopStartEnhancedHue);
            } else if (action === import_color_control.ColorControl.Action.ActivateTheColorLoopFromTheValueOfTheEnhancedCurrentHueAttribute) {
  ...
  moveColorTemperatureLogic(moveMode, rate, colorTemperatureMinimumMireds, colorTemperatureMaximumMireds) {
    if (!this.state.managedTransitionTimeHandling) {
      this.internal.stopColorTemperatureTransition();
      this.state.colorTemperatureMireds = moveMode === import_color_control.ColorControl.MoveMode.Up ? colorTemperatureMaximumMireds : colorTemperatureMinimumMireds;
      this.state.remainingTime = 0;
      return;
    }
    const effectiveRate = rate * (moveMode === import_color_control.ColorControl.MoveMode.Up ? 1 : -1);
    this.#initiateTransition(
      "colorTemperature",
      effectiveRate,
      colorTemperatureMinimumMireds,
  ...
  stepColorTemperatureLogic(stepMode, stepSize, transitionTime, colorTemperatureMinimumMireds, colorTemperatureMaximumMireds) {
    const targetValue = (0, import_general.cropValueRange)(
      this.state.colorTemperatureMireds + (stepMode === import_color_control.ColorControl.StepMode.Up ? stepSize : -stepSize),
      colorTemperatureMinimumMireds,
      colorTemperatureMaximumMireds
    );
    if (!this.state.managedTransitionTimeHandling || transitionTime === 0) {
  ...
  #getHueDistanceByDirection(currentHue, targetHue, direction, max) {
    ...
    if (direction === import_color_control.ColorControl.Direction.ShortestDistance) {
      if (Math.abs(distance) > max / 2) {
        return -(max - distance);
      }
      return distance;
    }
    if (direction === import_color_control.ColorControl.Direction.LongestDistance) {
      if (Math.abs(distance) > max / 2) {
        return distance;
      }
      return -(max - distance);
  ...
  #initializeManagedTransitionTimers() {
    ...
    this.internal.hueTransitionIntervalTimer = import_general.Time.getPeriodicTimer(
      "ColorControl.hue.transition",
      1e3,
      this.callback(this.#hueStepIntervalTick, { lock: true })
    );
    this.internal.saturationTransitionIntervalTimer = import_general.Time.getPeriodicTimer(
      "ColorControl.saturation.transition",
      1e3,
      this.callback(this.#saturationStepIntervalTick, { lock: true })
    );
    this.internal.colorTemperatureTransitionIntervalTimer = import_general.Time.getPeriodicTimer(
      "ColorControl.colorTemperature.transition",
      1e3,
      this.callback(this.#colorTemperatureStepIntervalTick, { lock: true })
    );
    this.internal.xTransitionIntervalTimer = import_general.Time.getPeriodicTimer(
      "ColorControl.x.transition",
      1e3,
      this.callback(this.#xStepIntervalTick, { lock: true })
    );
    this.internal.yTransitionIntervalTimer = import_general.Time.getPeriodicTimer(
  ...
  #initiateTransition(transitionType, changeRate, minValue, maxValue, targetValue) {
    ...
    if (transitionType === "enhancedHue" || transitionType === "hue") {
      if (targetValue !== void 0) {
        const direction = changeRate > 0 ? import_color_control.ColorControl.Direction.Up : import_color_control.ColorControl.Direction.Down;
  ...
  #stepIntervalTick(transitionType, transitionTimer, transitionData, currentValue) {
    ...
    switch (transitionType) {
      ...
      case "enhancedHue":
        ...
        newValue = (0, import_general.addValueWithOverflow)(
          currentValue,
          changeRate,
        ...
        break;
      case "saturation":
        newValue = (0, import_general.cropValueRange)(
      ...
      case "y":
        ...
        break;
      default:
        throw new import_general.ImplementationError(`Unknown transition type: ${transitionType}`);
    }
...
class ColorControlServer extends ColorControlServerLogic.for((0, import_types.ClusterType)(import_color_control.ColorControl.Base)) {
}
