...
class GeneralCommissioningServer extends import_GeneralCommissioningBehavior.GeneralCommissioningBehavior {
  ...
  async armFailSafe({ breadcrumb, expiryLengthSeconds }) {
    (0, import_protocol.assertSecureSession)(this.session, "armFailSafe can only be called on a secure session");
    const commissioner = this.env.get(import_protocol.DeviceCommissioner);
    try {
      if (!commissioner.isFailsafeArmed && this.agent.get(import_administrator_commissioning.AdministratorCommissioningServer).state.windowStatus !== import_administrator_commissioning2.AdministratorCommissioning.CommissioningWindowStatus.WindowNotOpen && !this.session.isPase) {
