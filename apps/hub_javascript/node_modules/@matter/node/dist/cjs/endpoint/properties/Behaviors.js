...
var import_Transaction = require("#behavior/state/transaction/Transaction.js");
...
class Behaviors {
  ...
  get detailedDiagnostic() {
    return Object.entries(this.#supported).map(([name, type]) => {
      ...
      const elements = new import_ValidatedElements.ValidatedElements(type);
  ...
  initialize() {
    ...
    const initializeBehaviors = (context) => {
      ...
      return import_general.Construction.all(
        {
          [Symbol.iterator]: () => {
  ...
  async close() {
    const dispose = async (context) => {
      ...
      if (transaction.status === import_Transaction.Transaction.Status.Exclusive) {
  ...
  internalsOf(type) {
    ...
    if (!backing) {
      ...
      if (backing === void 0) {
        throw new import_general.InternalError(`Behavior ${this.#endpoint}.${type.id} late activation did not create backing`);
      }
    }
  ...
  #activateLate(type) {
    const result = import_OfflineContext.OfflineContext.act(
      ...
      this.#endpoint.env.get(import_NodeActivity.NodeActivity),
  ...
  #augmentEndpoint(type) {
    Object.defineProperty(this.#endpoint.state, type.id, {
      ...
      set() {
        throw new import_general.ReadOnlyError('The "state" property is read-only; you must use set() to modify state');
      },
      enumerable: true
    });
    let events;
