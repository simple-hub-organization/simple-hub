...
class WindowCoveringServerLogic extends WindowCoveringServerBase {
  initialize() {
  ...
  #triggerLiftMotion(direction, targetPercent100ths) {
    this.#prepareMovement(0, direction, targetPercent100ths);
  }
  #triggerTiltMotion(direction, targetPercent100ths) {
    this.#prepareMovement(1, direction, targetPercent100ths);
  }
  /**
   ...
   */
  goToLiftValue({ liftValue }) {
    this.#assertMotionLockStatus();
    this.state.targetPositionLiftPercent100ths = this.#liftToPercent100ths(liftValue);
    this.#triggerLiftMotion(2 /* DefinedByPosition */, this.state.targetPositionLiftPercent100ths);
  }
  /**
   ...
   */
  goToTiltValue({ tiltValue }) {
    this.#assertMotionLockStatus();
    this.state.targetPositionTiltPercent100ths = this.#tiltToPercent100ths(tiltValue);
    this.#triggerTiltMotion(2 /* DefinedByPosition */, this.state.targetPositionTiltPercent100ths);
  }
  /**
   * Move the WindowCovering to a specific tilt value. The method calls the handleMovement method to actually move the
   * device to the defined position.
   ...
   */
  #convertValue(inputLowValue, inputHighValue, outputLowValue, outputHighValue, value) {
  ...
  #valueToPercent100ths(limits, absolute) {
  ...
  #percent100thsToValue(limits, relative) {
    return this.#convertValue(
      WC_PERCENT100THS_MIN_OPEN,
      WC_PERCENT100THS_MAX_CLOSED,
      limits.open,
...
((WindowCoveringServerLogic2) => {
  ...
  WindowCoveringServerLogic2.Internal = Internal;
  class State extends WindowCoveringServerBase.State {
    /** Does the device supports maintenance mode? */
    supportsMaintenanceMode = true;
  }
  WindowCoveringServerLogic2.State = State;
})(WindowCoveringServerLogic || (WindowCoveringServerLogic = {}));
class WindowCoveringServer extends WindowCoveringServerLogic.for(ClusterType(WindowCovering.Base)) {
}
export {
  CalibrationMode,
  MovementDirection,
