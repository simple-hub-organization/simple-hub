...
import { cropValueRange, Logger, MaybePromise, Time } from "#general";
...
class LevelControlServerLogic extends LevelControlLogicBase {
  ...
  moveLogic(moveMode, rate, withOnOff, options = {}) {
    ...
    if (!this.state.managedTransitionTimeHandling || effectiveRate === null || effectiveRate === 0) {
      const level = effectiveRate === 0 ? this.currentLevel : moveMode === LevelControl.MoveMode.Up ? this.maxLevel : this.minLevel;
      this.setRemainingTime(0);
      return this.setLevel(level, withOnOff, options);
    }
  ...
  stepLogic(stepMode, stepSize, transitionTime, withOnOff, options = {}) {
    ...
    const effectiveRate = Math.ceil(stepSize / transitionTime * 10) * (stepMode === LevelControl.StepMode.Up ? 1 : -1);
  ...
  setLevel(level, withOnOff, options = {}) {
    const onOffServer = this.features.onOff && withOnOff && this.agent.has(OnOffServer) ? this.agent.get(OnOffServer) : void 0;
...
class LevelControlServer extends LevelControlServerLogic.with(LevelControl.Feature.OnOff) {
}
export {
  LevelControlServer,
