...
function formatPlain(diagnostic, indents = 0) {
  const creator = plaintextCreator(indents);
  const formatter = {
    ...creator,
    message: (message) => {
      const formattedValues = ensureIndented(renderDiagnostic(message.values, formatter));
      return `${formatTime(message.now)} ${import_LogLevel.LogLevel[message.level]} ${message.facility} ${message.prefix}${formattedValues}`;
    },
...
function formatAnsi(diagnostic, indents = 0) {
  ...
  const formatter = {
    message: ({ now, level, facility, prefix: nestPrefix, values }) => {
      ...
      const prefix = style("prefix", `${formatTime(now)} ${import_LogLevel.LogLevel[level].padEnd(6)}`);
      facility = style(
        "facility",
...
function formatHtml(diagnostic) {
  ...
  const formatter = {
    message: ({ now, level, facility, prefix, values }) => {
      ...
      return htmlSpan(
        ...
        `${htmlSpan("time", formatTime(now))} ${htmlSpan("level", import_LogLevel.LogLevel[level])} ${htmlSpan(
          "facility",
          facility
