...
var import_StorageBackendDiskAsync = require("../storage/StorageBackendDiskAsync.js");
...
function configureStorage(env) {
  ...
  service.factory = (namespace) => new import_StorageBackendDiskAsync.StorageBackendDiskAsync((0, import_path.resolve)(service.location ?? ".", namespace), env.vars.get("storage.clear", false));
}
...
function loadConfigFile(vars) {
  ...
  let configJson;
  try {
    configJson = (0, import_fs.readFileSync)(configPath).toString();
  } catch (e) {
    throw new import_general.ImplementationError(`Error reading configuration file ${configPath}: ${e.message}`);
  }
  let configVars;
  try {
    configVars = JSON.parse(configJson);
  } catch (e) {
    throw new import_general.ImplementationError(`Error parsing configuration file ${configPath}: ${e.message}`);
  }
  return { configPath, configVars };
}
