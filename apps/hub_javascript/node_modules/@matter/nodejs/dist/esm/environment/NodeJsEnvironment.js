...
function configureStorage(env) {
  ...
  service.factory = (namespace) => new StorageBackendDiskAsync(resolve(service.location ?? ".", namespace), env.vars.get("storage.clear", false));
}
...
function loadConfigFile(vars) {
  ...
  try {
    ...
    throw new ImplementationError(`Error reading configuration file ${configPath}: ${e.message}`);
  }
  let configVars;
  try {
    configVars = JSON.parse(configJson);
  } catch (e) {
    throw new ImplementationError(`Error parsing configuration file ${configPath}: ${e.message}`);
  }
  return { configPath, configVars };
}
