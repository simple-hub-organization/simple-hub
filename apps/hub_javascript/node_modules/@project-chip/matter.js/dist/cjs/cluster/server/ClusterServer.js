 ...
 */
const logger = import_general.Logger.get("ClusterServer");
...
function ClusterServer(clusterDef, attributesInitialValues, handlers, supportedEvents = {}, ignoreMissingElements = false) {
  ...
  const result = {
    ...
    set datasource(newDatasource) {
      if (newDatasource === void 0) {
        throw new import_general.InternalError("Cluster datasource cannot be unset");
      }
      ...
      if (datasource.eventHandler) {
        for (const eventName in events) {
          ...
          if (bindResult !== void 0 && import_general.MaybePromise.is(bindResult)) {
            throw new import_general.InternalError("Binding events to event handler should never return a promise");
          }
        }
      }
    },
  ...
  for (const attributeName in attributeDef) {
    ...
    if (attributeDef[attributeName].isConditional) {
      ...
      if (mandatoryIf !== void 0 && mandatoryIf.length > 0) {
        ...
        if (conditionMatched && attributesInitialValues[attributeName] === void 0) {
          logger.warn(
            `InitialAttributeValue for "${clusterDef.name}/${attributeName}" is REQUIRED by supportedFeatures: ${import_general.Logger.toJSON(
              supportedFeatures
            )} but is not set!`
          );
        }
      ...
      if (!conditionHasMatched && optionalIf !== void 0 && optionalIf.length > 0) {
        ...
        if (conditionMatched && attributesInitialValues[attributeName] === void 0) {
          logger.debug(
            `InitialAttributeValue for "${clusterDef.name}/${attributeName}" is optional by supportedFeatures: ${import_general.Logger.toJSON(
              supportedFeatures
            )} and is not set!`
          );
        }
      ...
      if (!conditionHasMatched && attributesInitialValues[attributeName] !== void 0) {
        logger.warn(
          `InitialAttributeValue for "${clusterDef.name}/${attributeName}" is provided but it's neither optional or mandatory for supportedFeatures: ${import_general.Logger.toJSON(
            supportedFeatures
          )} but is set!`
        );
      }
    ...
    if (attributesInitialValues[attributeName] !== void 0) {
      ...
      attributes[attributeName] = (0, import_protocol.createAttributeServer)(
        clusterDef,
      ...
      attributeList.push((0, import_types.AttributeId)(id));
    } else {
  ...
  for (const name2 in commandDef) {
    ...
    if (commandDef[name2].isConditional) {
      ...
      if (mandatoryIf !== void 0 && mandatoryIf.length > 0) {
        ...
        if (conditionMatched && handler === void 0) {
          logger.warn(
            `Command "${clusterDef.name}/${name2}" is REQUIRED by supportedFeatures: ${import_general.Logger.toJSON(
              supportedFeatures
            )} but is not set!`
          );
        }
      ...
      if (!conditionHasMatched && optionalIf !== void 0 && optionalIf.length > 0) {
        ...
        if (conditionMatched && handler === void 0) {
          logger.debug(
            `Command "${clusterDef.name}/${name2}" is optional by supportedFeatures: ${import_general.Logger.toJSON(
              supportedFeatures
            )} and is not set!`
          );
        }
      ...
      if (!conditionHasMatched && handler !== void 0) {
        logger.warn(
          `Command "${clusterDef.name}/${name2}" is provided but it's neither optional nor mandatory for supportedFeatures: ${import_general.Logger.toJSON(
            supportedFeatures
          )} but is set!`
        );
      }
    ...
    commands[name2] = new import_protocol.CommandServer(
      requestId,
      responseId,
      name2,
      requestSchema,
  ...
  for (const eventName in eventDef) {
    ...
    if (eventDef[eventName].isConditional) {
      ...
      if (mandatoryIf !== void 0) {
        ...
        if (conditionMatched && supportedEvents[eventName] === void 0) {
          logger.warn(
            `Event "${clusterDef.name}/${eventName}" is REQUIRED by supportedFeatures: ${import_general.Logger.toJSON(
              supportedFeatures
            )} but is not set!`
          );
        }
      ...
      if (!conditionHasMatched && optionalIf !== void 0 && optionalIf.length > 0) {
        ...
        if (conditionMatched && supportedEvents[eventName] === void 0) {
          logger.debug(
            `Event "${clusterDef.name}/${eventName}" is optional by supportedFeatures: ${import_general.Logger.toJSON(
              supportedFeatures
            )} and is not set!`
          );
        }
      ...
      if (!conditionHasMatched && supportedEvents[eventName] !== void 0) {
        logger.warn(
          `Event "${clusterDef.name}/${eventName}" is provided but it's neither optional or mandatory for supportedFeatures: ${import_general.Logger.toJSON(
            supportedFeatures
          )} but is set!`
        );
      }
    ...
    if (supportedEvents[eventName] === true) {
      events[eventName] = (0, import_protocol.createEventServer)(
        clusterDef,
        eventDef[eventName],
        eventName,
        schema,
