...
class CommissioningController extends import_MatterNode.MatterNode {
  ...
  async removeNode(nodeId, tryDecommissioning = true) {
    ...
    if (tryDecommissioning) {
      try {
        if (node === void 0) {
          throw new import_general.ImplementationError(`Node ${nodeId} is not initialized.`);
        }
  ...
  async resetStorage() {
    ...
    if (environment !== void 0) {
      ...
      throw new import_general.InternalError("Storage not initialized correctly.");
    }
  }
...
async function configureNetwork(options) {
  ...
  const udpInterface = await import_general.UdpInterface.create(import_general.Network.get(), "udp6", localPort, listeningAddressIpv6);
  netInterfaces.add(udpInterface);
  if (!ipv4Disabled) {
    netInterfaces.add(await import_general.UdpInterface.create(import_general.Network.get(), "udp4", udpInterface.port, listeningAddressIpv4));
  }
  if (mdnsScanner) {
    scanners.add(mdnsScanner);
  }
  try {
    const ble = import_protocol.Ble.get();
    netInterfaces.add(ble.getBleCentralInterface());
    scanners.add(ble.getBleScanner());
  } catch (e) {
    if (e instanceof import_general.NoProviderError) {
