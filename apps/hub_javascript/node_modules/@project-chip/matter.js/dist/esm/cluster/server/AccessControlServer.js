...
const AccessControlClusterHandler = () => {
  ...
  return {
    ...
    aclAttributeSetter: (value, { session }) => {
      ...
      const oldValue = genericFabricScopedAttributeGetterFromFabric(
        fabric,
        AccessControlCluster,
        "acl",
        new Array()
      ...
      if (changed && accessControlEntryChangedEvent !== void 0) {
        ...
        for (; i < value.length; i++) {
          if (!isDeepEqual(value[i], oldValue[i])) {
            const changeType = oldValue[i] === void 0 ? AccessControl.ChangeType.Added : value[i] === void 0 ? AccessControl.ChangeType.Removed : AccessControl.ChangeType.Changed;
            accessControlEntryChangedEvent.triggerEvent({
              changeType,
              adminNodeId,
              adminPasscodeId,
              latestValue: (changeType === AccessControl.ChangeType.Removed ? oldValue[i] : value[i]) ?? null,
    ...
    extensionAttributeSetter: (value, { session }) => {
      ...
      if (changed && accessControlExtensionChangedEvent !== void 0) {
        const changeType = value.length > oldValue.length ? AccessControl.ChangeType.Added : value.length < oldValue.length ? AccessControl.ChangeType.Removed : AccessControl.ChangeType.Changed;
        const adminPasscodeId = session.isPase ? 0 : null;
        const adminNodeId = adminPasscodeId === null ? session.associatedFabric.rootNodeId : null;
...
const createDefaultAccessControlClusterServer = () => ClusterServer(
  AccessControlCluster,
  {
    acl: [],
    extension: [],
...
export {
  AccessControlClusterHandler,
  createDefaultAccessControlClusterServer
};
