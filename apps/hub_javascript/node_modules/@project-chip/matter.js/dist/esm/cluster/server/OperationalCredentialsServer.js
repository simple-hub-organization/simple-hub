...
const OperationalCredentialsClusterHandler = (certificates, productDescription) => {
  ...
  return {
    ...
    trustedRootCertificatesAttributeGetter: ({ session }) => {
      ...
      if (!session.isSecure)
        throw new MatterFlowError("addOperationalCert should be called on a secure session.");
      const rootCerts = MatterDevice.of(session).getFabrics().map((fabric) => fabric.rootCert);
      const device = MatterDevice.of(session);
    ...
    currentFabricIndexAttributeGetter: ({ session }) => {
      if (session === void 0 || !session.isSecure) return FabricIndex.NO_FABRIC;
      assertSecureSession(session);
    ...
    updateFabricLabel: async ({ request: { label }, attributes: { fabrics }, session }) => {
      ...
      const conflictingLabelFabric = MatterDevice.of(session).getFabrics().find((f) => f.label === label && f.fabricIndex !== currentFabricIndex);
